...
function jigoshop_new_order_notification( $order_id ) {
	...
	if (count($totals) > 0) echo PHP_EOL;
	foreach($totals as $total){
		echo $total->get_title_display(false) . "\t\t\t" . $total->get_amount_display(false) . PHP_EOL;
	}

	echo PHP_EOL.__('Payment Method', 'jigoshop') . "\t\t\t" . ucwords($order->payment_method) . PHP_EOL . PHP_EOL;
